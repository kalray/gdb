				   ...
				   Elf_Internal_Sym *sym)
{
  ...
  if (h != NULL
      ...
      && h->def_regular)
    {
      ...
      /* FIXME K1 : this is not yet handled (ie. it has been sed-ed and never tested */
      BFD_ASSERT(0);
	...
	case BFD_RELOC_K1_64:
	  ...
	  if (bfd_link_pic (info) && h->non_got_ref)
		  ...
		  || bfd_link_executable (info))
		{
		  ...
		  outrel.r_info = ELFNN_R_INFO (0, K1_R (IRELATIVE));
	...
	case BFD_RELOC_AARCH64_LD64_GOT_LO12_NC:
	  ...
	  if (off == (bfd_vma) -1)
	    ...
	    value = k1_calculate_got_entry_vma (h, globals, info,
						     value, output_bfd,
						     unresolved_reloc_p);

	  /* FIXME K1 : This is new, this may cause problem */
	  switch (bfd_r_type)
	    {
	    case BFD_RELOC_AARCH64_LD32_GOTPAGE_LO14:
	    case BFD_RELOC_AARCH64_LD64_GOTPAGE_LO15:
	  ...
	  value = _bfd_k1_elf_resolve_relocation (bfd_r_type, place, value,
						       addend, weak_undef_p);
	  return _bfd_k1_elf_put_addend (input_bfd, hit_data, bfd_r_type, howto, value);

	  /* FIXME K1 : This most likely has to go away :) */
  ...
  for (; rel < relend; rel++)
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  if (!bfd_link_relocatable (info)
	    ...
	    {
	      h = elfNN_k1_get_local_sym_hash (globals, input_bfd,
						    rel, FALSE);
	      if (h == NULL)
		abort ();
  ...
  for (rel = relocs; rel < relend; rel++)
      ...
      else
	{
	  ...
	  if (isym != NULL
	    ...
	    {
	      h = elfNN_k1_get_local_sym_hash (htab, abfd, rel, FALSE);
	      if (h == NULL)
		abort ();
	    }
#endif /* K1_DISABLE_IFUNC */
  ...
  for (rel = relocs; rel < rel_end; rel++)
      ...
      if (r_symndx < symtab_hdr->sh_info)
	{
	  ...
	  if (ELF_ST_TYPE (isym->st_info) == STT_GNU_IFUNC)
	    {
	      h = elfNN_k1_get_local_sym_hash (htab, abfd, rel,
						    TRUE);
	      if (h == NULL)
		return FALSE;
...
elfNN_k1_allocate_local_ifunc_dynrelocs (void **slot, void *inf)
{
  ...
  return elfNN_k1_allocate_ifunc_dynrelocs (h, inf);
}
#endif /* K1_DISABLE_IFUNC */
...
#if ! K1_DISABLE_IFUNC
  elf_link_hash_traverse (&htab->root, elfNN_k1_allocate_ifunc_dynrelocs,
			  info);
#endif /* K1_DISABLE_IFUNC */
...
#if ! K1_DISABLE_IFUNC
  htab_traverse (htab->loc_hash_table,
		 elfNN_k1_allocate_local_ifunc_dynrelocs,
		 info);
#endif /* K1_DISABLE_IFUNC */
...
/* #if ARCH_SIZE == 32 */
  ...
  elf_k1_update_plt_entry(output_bfd, BFD_RELOC_K1_S37_UP27,
			  ...
			  gotplt_entry_address - plt_entry_address);
/* #else */
/*   /\* Fill the LO10 of of lw $r10r11 = 0[$r14r15] *\/ */
/*   elf_k1_update_plt_entry(output_bfd, BFD_RELOC_K1_S64_LO10, */
/* 			  plt_entry+4, */
/* 			  gotplt_entry_address - plt_entry_address); */

/*   /\* Fill the UP27 of of lw $r10r11 = 0[$r14r15] *\/ */
/*   elf_k1_update_plt_entry(output_bfd, BFD_RELOC_K1_S64_UP27, */
/* 			  plt_entry+8, */
/* 			  gotplt_entry_address - plt_entry_address); */

/*   /\* Fill the EX27 of of lw $r10r11 = 0[$r14r15] *\/ */
/*   elf_k1_update_plt_entry(output_bfd, BFD_RELOC_K1_S64_EX27, */
/* 			  plt_entry+12, */
/* 			  gotplt_entry_address - plt_entry_address); */
/* #endif */
  ...
  /* elf_k1_update_plt_entry (output_bfd, BFD_RELOC_AARCH64_ADR_HI21_PCREL, */
  /* 				plt_entry, */
  /* 				PG (gotplt_entry_address) - */
  /* 				PG (plt_entry_address)); */

  /* /\* Fill in the lo12 bits for the load from the pltgot.  *\/ */
  /* elf_k1_update_plt_entry (output_bfd, BFD_RELOC_AARCH64_LDSTNN_LO12, */
  /* 				plt_entry + 4, */
  /* 				PG_OFFSET (gotplt_entry_address)); */

  /* /\* Fill in the lo12 bits for the add from the pltgot entry.  *\/ */
  /* elf_k1_update_plt_entry (output_bfd, BFD_RELOC_AARCH64_ADD_LO12, */
  /* 				plt_entry + 8, */
  /* 				PG_OFFSET (gotplt_entry_address)); */

  /*
   * Lazy resolution not supported yet. Instead, insert a BFD_R_K1_NN
   * reloc on the corresponding symbol in the GOT
   */

  /*
   * FIXME : K1 does not support lazy resolution yet.
   * PLT0 is not really used.
   */
  ...
  if (h->dynindx == -1
	  ...
	  && h->type == STT_GNU_IFUNC))
    {
	 ...
	 R_AARCH64_IRELATIVE instead of R_AARCH64_JUMP_SLOT.  */
      rela.r_info = ELFNN_R_INFO (0, K1_R (IRELATIVE));
