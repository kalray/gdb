...
k1_unsigned_overflow (bfd_vma value, unsigned int bits)
{
  bfd_vma lim;
...
static bfd_reloc_status_type
k1_signed_overflow (bfd_vma value, unsigned int bits)
{
...
_bfd_k1_elf_put_addend (bfd *abfd,
			     ...
			     reloc_howto_type *howto, bfd_signed_vma addend)
{
  ...
  switch (howto->complain_on_overflow)
    ...
    case complain_overflow_signed:
      status = k1_signed_overflow (addend,
					howto->bitsize + howto->rightshift);
      break;
    case complain_overflow_unsigned:
      status = k1_unsigned_overflow (addend,
					  howto->bitsize + howto->rightshift);
      break;
  ...
  /* FIXME K1 : AARCH64 is "redoing" what the link_relocate bfd
   ...
   * against a input_bfd (eg. when doing PLT).  On K1, we have not
...
bfd_boolean
_bfd_k1_elf_add_symbol_hook (bfd *abfd, struct bfd_link_info *info,
