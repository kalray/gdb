...
b.target("build") do 
  ...
  if( arch == "k1" )
    ...
    b.run(:cmd => "../configure --target=#{build_target} --program-prefix=#{arch}- --disable-werror --with-python=$PWD/../bundled_libraries/python --with-libexpat-prefix=$PWD/../bundled_libraries/expat --prefix=#{build_path}/release LDFLAGS=\"-lutil -pthread -rdynamic\" CFLAGS=\"-pthread\"")
