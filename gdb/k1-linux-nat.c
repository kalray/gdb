/* Kalray K1 GNU/Linux native support.
...
static const struct inferior_data *k1_linux_inferior_data_token;
...
k1_linux_inferior_data (int pid)
{
  ...
  res = inferior_data (inf, k1_linux_inferior_data_token);
  if (!res)
    {
      res = xcalloc (1, sizeof (*res));
      set_inferior_data (inf, k1_linux_inferior_data_token, res);
...
static void
k1_get_gregs (int tid, gdb_gregset_t *regs)
{
  struct iovec iovec;
...
static void
k1_linux_fetch_inferior_registers (struct target_ops *ops,
...
static void
k1_linux_store_inferior_registers (struct target_ops *ops,
				   struct regcache *regcache, int regnum)
{
...
static int
k1_linux_can_use_hw_breakpoint (struct target_ops *self, enum bptype type,
...
static int
k1_get_hw_point_type_info (int type, struct k1_linux_hw_pt **pts, int *count)
{
  ...
  *count = 0;
  ...
  data = k1_linux_inferior_data (pid);
  if (!data)
    return -1;
...
k1_get_thread_info (struct lwp_info *lwp)
{
  int i;
...
static int
k1_update_thread_inf_cb (struct lwp_info *lwp, void *arg)
{
  struct update_registers_data *cb_data = (struct update_registers_data *) arg;
  struct arch_lwp_info *info = k1_get_thread_info (lwp);
...
static int
k1_linux_insert_hw_point (CORE_ADDR addr, int len, int type)
{
  ...
  if (k1_get_hw_point_type_info (type, &pts, &count))
    return -1;
  ...
  for (i = 0; i < count; i++)
      ...
      if (!pts[i].enabled)
	{
	  ...
	  iterate_over_lwps (pid_to_ptid (pid), k1_update_thread_inf_cb,
			     &cb_data);
...
static int
k1_linux_insert_hw_breakpoint (struct target_ops *self, struct gdbarch *gdbarch,
			       struct bp_target_info *bp_tgt)
{
  ...
  return k1_linux_insert_hw_point (addr, len, HW_BKP_TYPE);
}
...
static int
k1_linux_insert_watchpoint (struct target_ops *self, CORE_ADDR addr, int len,
			    ...
			    struct expression *cond)
{
  ...
  return k1_linux_insert_hw_point (addr, len, HW_WRITE_WP_TYPE);
}

static int
k1_linux_remove_hw_point (CORE_ADDR addr, int len, int type)
{
  struct k1_linux_hw_pt *pts;
  int i, found, count;
  int pid = ptid_get_pid (inferior_ptid);
...
/* Remove a hardware-assisted breakpoint at BP_TGT->placed_address.
   Return 0 on success, -1 on failure. */
static int
k1_linux_remove_hw_breakpoint (struct target_ops *self, struct gdbarch *gdbarch,
			       struct bp_target_info *bp_tgt)
{
  return k1_linux_remove_hw_point (bp_tgt->placed_address, bp_tgt->length,
				   HW_BKP_TYPE);
}
...
static int
k1_linux_remove_watchpoint (struct target_ops *self, CORE_ADDR addr, int len,
			    enum target_hw_bp_type type,
			    struct expression *cond)
{
  return k1_linux_remove_hw_point (addr, len, HW_WRITE_WP_TYPE);
}

static int
k1_linux_stopped_data_address (struct target_ops *ops, CORE_ADDR *addr_p)
{
  siginfo_t siginfo;
  int tid = ptid_get_lwp (inferior_ptid);
...
static int
k1_linux_stopped_by_watchpoint (struct target_ops *ops)
{
  CORE_ADDR addr;

  return k1_linux_stopped_data_address (ops, &addr);
}

/* Function to call when a new thread is detected. */
static void
k1_linux_new_thread (struct lwp_info *lwp)
{
  k1_get_thread_info (lwp);
}

/* Called when resuming a thread LWP.
   The hardware debug registers are updated when there is any change. */
static void
k1_linux_prepare_to_resume (struct lwp_info *lwp)
{
  struct arch_lwp_info *info = k1_get_thread_info (lwp);
  int i, j, ret;
		    ...
		    void **base)
{
  ...
  k1_get_gregs (lwpid, &regs);

  /* IDX is the bias from the thread pointer to the beginning of the
   thread descriptor.  It has to be subtracted due to implementation
   quirks in libthread_db. */
  *base = (void *) (uintptr_t) (regs[R_TLS] - idx - K1_TLS_TCB_SIZE);

  return 0;
}

static void
k1_cleanup_linux_inferior_data (struct inferior *inf, void *data)
{
  xfree (data);
}

void _initialize_k1_linux_nat (void);
void
_initialize_k1_linux_nat (void)
{
  struct target_ops *t;
  ...
  t->to_fetch_registers = k1_linux_fetch_inferior_registers;
  t->to_store_registers = k1_linux_store_inferior_registers;
  t->to_can_use_hw_breakpoint = k1_linux_can_use_hw_breakpoint;
  t->to_insert_hw_breakpoint = k1_linux_insert_hw_breakpoint;
  t->to_remove_hw_breakpoint = k1_linux_remove_hw_breakpoint;
  t->to_insert_watchpoint = k1_linux_insert_watchpoint;
  t->to_remove_watchpoint = k1_linux_remove_watchpoint;
  t->to_stopped_by_watchpoint = k1_linux_stopped_by_watchpoint;
  t->to_stopped_data_address = k1_linux_stopped_data_address;

  linux_nat_add_target (t);
  linux_nat_set_new_thread (t, k1_linux_new_thread);
  linux_nat_set_prepare_to_resume (t, k1_linux_prepare_to_resume);
  ...
  k1_linux_inferior_data_token
    = register_inferior_data_with_cleanup (NULL,
					   k1_cleanup_linux_inferior_data);
}
